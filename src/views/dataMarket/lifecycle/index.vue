<template>
  ...
  <ContentWrap>
    <el-table v-loading="applicationLoading" :data="applications" empty-text="暂无变更、续期或停用申请">
      ...
      <el-table-column label="操作" width="110" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" @click="reviewApplication(row.id)">影响审查</el-button>
        </template>
      </el-table-column>
    </el-table>
  </ContentWrap>
  ...
  <ContentWrap>
    ...
    <template v-else-if="application">
      <el-descriptions title="申请与影响审查" :column="responsiveColumns" border class="mb-16px">
        ...
        <el-descriptions-item label="申请类型"><el-tag>{{ application.applicationType }}</el-tag></el-descriptions-item>
        <el-descriptions-item label="申请状态">{{ application.status }}</el-descriptions-item>
        <el-descriptions-item label="原 API">{{ lifecycleRequest?.originalApiId ?? '服务端未返回' }}</el-descriptions-item>
        <el-descriptions-item label="原版本">{{ lifecycleRequest?.originalApiVersionId ?? '服务端未返回' }}</el-descriptions-item>
        <el-descriptions-item label="执行状态">{{ lifecycleRequest?.executionStatus ?? '服务端未返回' }}</el-descriptions-item>
        <el-descriptions-item label="申请生效时间">{{ lifecycleRequest?.desiredEffectiveAt || '未设置' }}</el-descriptions-item>
        <el-descriptions-item label="申请使用截止">{{ lifecycleRequest?.requestedUseEndDate || '未设置' }}</el-descriptions-item>
        <el-descriptions-item label="已关联凭证">
          <el-tag v-for="id in lifecycleRequest?.affectedCredentialIds || []" :key="id" class="mr-4px">{{ id }}</el-tag>
          <span v-if="!lifecycleRequest?.affectedCredentialIds?.length">无</span>
        </el-descriptions-item>
        <el-descriptions-item label="服务端影响摘要" :span="responsiveColumns">{{ lifecycleRequest?.impactSummary || '待评估' }}</el-descriptions-item>
        <el-descriptions-item label="已有执行结果" :span="responsiveColumns">{{ lifecycleRequest?.executionResult || '尚未执行' }}</el-descriptions-item>
      </el-descriptions>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="140px" class="max-w-760px">
        ...
        <el-form-item label="近期调用摘要" prop="recentCallSummary">
          <el-input v-model="form.recentCallSummary" type="textarea" :rows="3" placeholder="审查近期调用量、失败率与调用方" />
        </el-form-item>
        <el-form-item label="已确认影响" prop="confirmedImpactSummary">
          <el-input v-model="form.confirmedImpactSummary" type="textarea" :rows="3" placeholder="确认调用方、数据与变更窗口影响" />
        </el-form-item>
        <el-form-item label="计划/实际生效时间" prop="actualEffectiveAt">
          <el-date-picker v-model="form.actualEffectiveAt" type="datetime" value-format="YYYY-MM-DDTHH:mm:ssZ" />
        </el-form-item>
        <el-form-item label="关联凭证确认">
          <el-select v-model="affectedCredentialIds" multiple filterable allow-create default-first-option class="!w-100%" placeholder="输入并确认受影响凭证 ID">
            <el-option v-for="id in lifecycleRequest?.affectedCredentialIds || []" :key="id" :label="String(id)" :value="String(id)" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="application.applicationType === 'CHANGE'" label="目标版本 ID" prop="targetApiVersionId">
        ...
        <el-form-item label="执行动作" prop="actionDetail">
          <el-input v-model="form.actionDetail" type="textarea" :rows="3" placeholder="说明切换、续期或停用执行动作" />
        </el-form-item>
        <el-form-item label="执行结果">
          <el-input v-model="form.executionResult" type="textarea" :rows="3" placeholder="立即执行时填写；计划执行由后台完成后回填" />
        </el-form-item>
        <el-form-item>
          <el-button v-hasPermi="['data-market:lifecycle:execute']" type="danger" :loading="submitting" @click="execute">确认执行</el-button>
        </el-form-item>
      </el-form>
    </template>
  </ContentWrap>
...
import type { ApplicationDetailVO, ApplicationSummaryVO, LifecycleRequestDetail } from '@/api/dataMarket/types'
...
const lifecycleRequest = computed<LifecycleRequestDetail | undefined>(() => application.value?.lifecycleRequest)
const form = reactive({
